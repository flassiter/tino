...
class SaveFileDialogScreen(ModalScreen):
    ...
    def __init__(
        self, initial_path: Path | None = None, default_name: str = "untitled.md"
    ) -> None:
        super().__init__()
...
class PreviewApp(App[None]):
    ...
    async def action_save_file_as(self) -> None:
        ...
        def handle_save_file_selected(save_path: Path | None) -> None:
    ...
    async def _save_to_file(self, file_path: Path) -> bool:
        ...
        try:
            success = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: self._file_manager.save_file(
                    str(file_path), self._editor_pane.content
                ),
            )
            return success
        except Exception:
