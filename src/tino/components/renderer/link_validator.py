...
class LinkValidator:
    ...
    def find_all_links(self, content: str) -> list[dict[str, Any]]:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            for match in re.finditer(self._link_patterns["reference"], line):
                ref_key = match.group(2) or match.group(
                    1
                )  # Use text as ref if ref is empty
    ...
    def validate_links(
        ...
        for link in links:
            issues.extend(
                self._validate_single_link(link, base_path, headings, check_external)
            )
    ...
    def suggest_fragment_corrections(
        self, fragment: str, headings: list[Heading]
    ) -> list[str]:
        """
        ...
        if parsed.scheme in ("http", "https"):
            ...
            if url.startswith("#"):
                ...
                if not self.check_fragment_exists(fragment, headings):
                    ...
                    if suggestions:
                        suggestion_text = (
                            f" Did you mean: {', '.join(suggestions[:2])}?"
                        )
