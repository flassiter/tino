...
from .command_base import CommandContext, FileCommand
...
class FileSwitcher:
    ...
    def get_recent_files(self, limit: int | None = None) -> list[FileInfo]:
        """
        ...
        for path in recent_paths:
            try:
                if self._file_manager.file_exists(path):
                    size, modified_time, encoding = self._file_manager.get_file_info(
                        path
                    )
    ...
    def start_tab_cycle(self) -> Path | None:
        """
    ...
    def continue_tab_cycle(self) -> Path | None:
        """
    ...
    def end_tab_cycle(self) -> Path | None:
        """
        ...
        selected_file = None
        if self._tab_cycle_files and 0 <= self._tab_cycle_index < len(
...
class QuickSwitchCommand(FileCommand):
    ...
    def __init__(self, context: CommandContext | None = None):
        ...
        self._file_switcher: FileSwitcher | None = None
...
class RecentFilesDialogCommand(FileCommand):
    ...
    def execute(self, *args: Any, **kwargs: Any) -> bool:
        ...
        try:
            ...
            raise CommandError(f"Failed to show recent files: {e}", self.get_name(), e) from e
...
class SwitchToFileCommand(QuickSwitchCommand):
    ...
    def execute(self, *args: Any, **kwargs: Any) -> bool:
        ...
        try:
            ...
            raise CommandError(f"Failed to switch to file: {e}", self.get_name(), e) from e
