...
from ...core.events.types import CursorMovedEvent, ReplaceEvent, SearchEvent
from ...core.interfaces.command import CommandError
from .categories import CommandCategory
from .command_base import EditorCommand
...
class FindCommand(EditorCommand):
    ...
    def execute(self, *args: Any, **kwargs: Any) -> bool:
        ...
        try:
            ...
            # Perform search
            match_range = self._find_text(
                pattern, start_pos, case_sensitive, whole_word
            )
            ...
            if match_range:
                ...
                all_matches = self._find_all_matches(
                    pattern, case_sensitive, whole_word
                )
            ...
            raise CommandError(f"Failed to find text: {e}", self.get_name(), e) from e
    ...
    def _find_text(
        self, pattern: str, start: int, case_sensitive: bool, whole_word: bool
    ) -> tuple[int, int] | None:
    ...
    def _find_all_matches(
        self, pattern: str, case_sensitive: bool, whole_word: bool
    ) -> list[tuple[int, int]]:
...
class FindNextCommand(EditorCommand):
    ...
    def execute(self, *args: Any, **kwargs: Any) -> bool:
        ...
        try:
            ...
            raise CommandError(f"Failed to find next: {e}", self.get_name(), e) from e
    ...
    def can_execute(self, *args: Any, **kwargs: Any) -> bool:
        ...
        return super().can_execute(*args, **kwargs) and bool(
            self.context.application_state.get("search_results", [])
        )
...
class FindPreviousCommand(EditorCommand):
    ...
    def execute(self, *args: Any, **kwargs: Any) -> bool:
        ...
        try:
            ...
            raise CommandError(f"Failed to find previous: {e}", self.get_name(), e) from e
...
class ReplaceCommand(EditorCommand):
    ...
    def execute(self, *args: Any, **kwargs: Any) -> bool:
        ...
        try:
            ...
            if replace_all:
                ...
                replacements_made = self._replace_all(
                    pattern, replacement, case_sensitive, whole_word
                )
            else:
                # Replace current selection or find and replace next occurrence
                replacements_made = self._replace_current(
                    pattern, replacement, case_sensitive, whole_word
                )
            ...
            raise CommandError(f"Failed to replace text: {e}", self.get_name(), e) from e
    ...
    def _replace_all(
        self, pattern: str, replacement: str, case_sensitive: bool, whole_word: bool
    ) -> int:
    ...
    def _replace_current(
        ...
        if selected_text and self._matches_pattern(
            selected_text, pattern, case_sensitive, whole_word
        ):
            ...
            cursor_pos = self.editor.get_cursor_position()
            match_range = self._find_text(
                pattern, cursor_pos[2], case_sensitive, whole_word
            )
    ...
    def _matches_pattern(
        self, text: str, pattern: str, case_sensitive: bool, whole_word: bool
    ) -> bool:
        ...
        try:
            ...
            if isinstance(line_number, str):
                try:
                    ...
                    raise CommandError("Invalid line number format", self.get_name()) from e
            ...
            raise CommandError(f"Failed to go to line: {e}", self.get_name(), e) from e
