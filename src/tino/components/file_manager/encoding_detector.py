...
class EncodingDetector:
    ...
    def detect_file_encoding(self, file_path: Path) -> str:
        ...
        try:
            ...
            logger.warning(
                f"Could not reliably detect encoding for {file_path}, using latin-1"
            )
            return "latin-1"
    ...
    def _detect_with_chardet(self, data: bytes) -> str | None:
        """
        ...
        try:
            ...
            if result and result.get("encoding"):
                ...
                logger.debug(
                    f"Chardet result: {encoding} (confidence: {confidence:.2f})"
                )
    ...
    def _detect_with_fallbacks(self, data: bytes) -> str | None:
        """
    ...
    def detect_bytes_encoding(self, data: bytes) -> tuple[str, float]:
        ...
        for encoding in self.FALLBACK_ENCODINGS:
            if self._can_decode(data, encoding):
                ...
                confidence = max(
                    0.1, 1.0 - (self.FALLBACK_ENCODINGS.index(encoding) * 0.1)
                )
                return encoding, confidence
    ...
    def get_bom_encoding(self, data: bytes) -> str | None:
        """
