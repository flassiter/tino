...
from tino.core.events.types import FileClosedEvent, FileOpenedEvent, FileSavedEvent
...
class FileManager(IFileManager):
    ...
    def open_file(self, file_path: Path) -> str:
        ...
        try:
            ...
            if file_size > self.LARGE_FILE_THRESHOLD:
                logger.warning(
                    f"Opening large file ({file_size / 1024 / 1024:.1f}MB): {file_path}"
                )
        except OSError as e:
    ...
    def save_file(
        ...
        try:
            ...
            logger.info(
                f"Saved file: {file_path} ({encoding}, {len(content)} chars, backup: {backup_created})"
            )
            return True
    ...
    def get_temp_file_path(self, original_path: Path) -> Path:
        ...
        with tempfile.NamedTemporaryFile(
            dir=parent, prefix=prefix, suffix=suffix, delete=False
        ) as f:
            return Path(f.name)
    ...
    def close_file(
        self, file_path: Path, was_modified: bool = False, saved: bool = False
    ) -> None:
        """
        ...
        logger.debug(
            f"Closed file: {file_path} (modified: {was_modified}, saved: {saved})"
        )
