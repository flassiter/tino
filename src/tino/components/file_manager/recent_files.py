    ...
    def set_max_files(self, max_files: int) -> None:
        ...
        with self._lock:
            ...
            while len(self._files) > max_files:
                oldest_file, _ = self._files.popitem(last=True)
                logger.debug(
                    f"Removed old recent file due to limit change: {oldest_file}"
                )
    ...
    def get_stats(self) -> dict:
        ...
        with self._lock:
            return {
                ...
                "oldest_access_time": (
                    min(self._files.values()) if self._files else None
                ),
                "newest_access_time": (
                    max(self._files.values()) if self._files else None
                ),
            }
