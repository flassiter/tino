...
class CursorMemory:
    ...
    def set_cursor_position(self, file_path: Path, line: int, column: int) -> None:
        ...
        with self._lock:
            ...
            if old_position != (line, column):
                logger.debug(f"Updated cursor position for {file_path}: {line}:{column}")
    ...
    def get_cursor_position(self, file_path: Path) -> Optional[Tuple[int, int]]:
        ...
        with self._lock:
            ...
            if position:
                logger.debug(f"Retrieved cursor position for {file_path}: {position[0]}:{position[1]}")
            return position
    ...
    def update_cursor_position(self, file_path: Path, delta_line: int = 0, delta_column: int = 0) -> Optional[Tuple[int, int]]:
        """
        ...
        with self._lock:
            ...
            logger.debug(f"Updated cursor position for {file_path} by delta ({delta_line}, {delta_column}): {new_line}:{new_column}")
    ...
    def cleanup_missing_files(self) -> int:
        ...
        if removed_count > 0:
            logger.info(f"Cleaned up cursor positions for {removed_count} missing files")
    ...
    def get_stats(self) -> dict:
        ...
        with self._lock:
            ...
            return {
                ...
                'avg_line': sum(pos[0] for pos in positions) / len(positions) if positions else None,
                'avg_column': sum(pos[1] for pos in positions) / len(positions) if positions else None,
            }
    
    def validate_position(self, file_path: Path, max_lines: Optional[int] = None) -> Optional[Tuple[int, int]]:
        """
