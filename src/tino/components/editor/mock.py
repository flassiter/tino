...
class MockEditor(IEditor):
    ...
    def insert_text(self, position: int, text: str) -> None:
        ...
        self._emit_text_changed(self._content, old_content, "insert", position, len(text))
    ...
    def delete_range(self, start: int, end: int) -> str:
        ...
        self._record_operation("delete_range", start=start, end=end, deleted_text=deleted_text)
        self._emit_text_changed(self._content, old_content, "delete", start, end - start)
    ...
    def set_selection(self, start: int, end: int) -> None:
        ...
        self._record_operation("set_selection", start=start, end=end, old_selection=old_selection)
    ...
    def get_cursor_position(self) -> Tuple[int, int, int]:
        ...
        absolute_pos = self._calculate_absolute_position(self._cursor_line, self._cursor_column)
    ...
    def set_cursor_position(self, line: int, column: int) -> None:
        ...
        self._record_operation("set_cursor_position", line=line, column=column, old_position=old_position)
        
        if old_position != (line, column):
            self._emit_cursor_moved(line, column, new_abs, old_position[0], old_position[1], old_abs)
    ...
    def replace_selection(self, text: str) -> None:
        ...
        if start == end:
            ...
            self._record_operation("replace_selection", text=text, start=start, end=end, selected_text=selected_text)
            self._emit_text_changed(self._content, old_content, "replace", start, len(selected_text))
    ...
    def find_text(self, pattern: str, start: int = 0, case_sensitive: bool = True) -> Optional[Tuple[int, int]]:
        ...
        if self._simulate_find_failures:
            self._record_operation("find_text", pattern=pattern, start=start, case_sensitive=case_sensitive, result=None)
            return None
        ...
        self._record_operation("find_text", pattern=pattern, start=start, case_sensitive=case_sensitive, result=result)
        return result
    ...
    def set_modified(self, modified: bool) -> None:
        ...
        self._record_operation("set_modified", modified=modified, old_modified=old_modified)
    ...
    def clear_undo_history(self) -> None:
        ...
        self._record_operation("clear_undo_history", old_undo_count=old_undo_count, old_redo_count=old_redo_count)
    ...
    def set_simulate_failures(self, find_failures: bool = False, undo_failures: bool = False) -> None:
    ...
    def _apply_undo_operation(self, operation: Dict[str, Any]) -> None:
        ...
        if op_type == 'set_content':
            ...
            self._content = self._content[:start] + old_text + self._content[current_end:]
            self._cursor_line, self._cursor_column = operation['old_cursor']
    ...
    def _update_cursor_after_insert(self, position: int, text: str) -> None:
        ...
        abs_cursor = self._calculate_absolute_position(self._cursor_line, self._cursor_column)
    ...
    def _emit_text_changed(self, content: str, old_content: str, change_type: str, position: int, length: int) -> None:
    ...
    def _emit_cursor_moved(self, line: int, column: int, position: int, old_line: int, old_column: int, old_position: int) -> None:
