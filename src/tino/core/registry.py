...
from .events import ComponentLoadedEvent, ComponentUnloadedEvent, EventBus
...
class ComponentRegistry:
    ...
    def get_component(self, name: str, component_type: type[T] | None = None) -> T:
        """
        ...
        with self._lock:
            ...
            if (
                ...
                and self._singleton_instances[name] is not None
            ):
    ...
    def _create_component(
        ...
        try:
            ...
            if expected_type and not isinstance(instance, expected_type):
                raise TypeError(
                    f"Factory for {name} returned {type(instance)}, expected {expected_type}"
                )
            ...
            raise ComponentCreationError(
                f"Failed to create component '{name}': {e}"
            ) from e
        finally:
    ...
    def resolve_initialization_order(self) -> list[str]:
        ...
        in_degree = dict.fromkeys(self._component_types, 0)
    ...
    def get_component_info(self, name: str) -> dict[str, Any]:
        ...
        return {
            ...
            "factory": (
                self._factories[name].__name__ if self._factories.get(name) else None
            ),
        }
...
if __name__ == "__main__":
    ...
    print("\n=== Demo Complete ===")
    print(
        f"Registry successfully demonstrated dependency injection with {len(loaded_components)} components!"
    )
