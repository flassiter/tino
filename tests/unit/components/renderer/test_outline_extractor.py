...
#### Title Four ####"""
    ...
    def test_get_section_range(self, extractor):
        ...
        headings = [
            ...
            Heading(level=3, text="Sub A.1", id="sub-a-1", line_number=8),
            Heading(
                level=2, text="Sub B", id="sub-b", line_number=15
            ),  # Same level as Sub A
            Heading(
                level=1, text="Main 2", id="main-2", line_number=20
            ),  # Same level as Main
        ]
...
##### Título çom açentos"""
    ...
    def test_performance_large_document(self, extractor):
        ...
        assert (
            extraction_time < 100
        ), f"Extraction took {extraction_time:.2f}ms, should be <100ms"
    ...
    def test_toc_generation_large_hierarchy(self, extractor):
        ...
        for i in range(10):  # 10 main sections
            headings.append(
                Heading(level=1, text=f"Main {i}", id=f"main-{i}", line_number=line_num)
            )
            line_num += 5
