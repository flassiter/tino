...
"""
    ...
    def test_html_export(self, renderer, sample_markdown):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            success = renderer.export_html(
                sample_markdown, str(output_path), standalone=True, include_css=True
            )
    ...
    def test_html_export_minimal(self, renderer, sample_markdown):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            success = renderer.export_html(
                sample_markdown, str(output_path), standalone=False, include_css=False
            )
...
"""
    ...
    def test_performance_requirement(self, renderer):
        ...
        renderer.render_html(content)  # Just verify it works
        ...
        assert (
            render_time_ms < 50
        ), f"Render time {render_time_ms:.2f}ms exceeds 50ms requirement"
    ...
    def test_large_document_performance(self, renderer):
        ...
        assert (
            render_time_ms < 200
        ), f"Large document render time {render_time_ms:.2f}ms too slow"
    ...
    def test_heading_id_generation(self, renderer):
        ...
        for text, expected_id in test_cases:
            ...
            if headings:
                assert (
                    headings[0].id == expected_id
                ), f"Expected '{expected_id}' for '{text}', got '{headings[0].id}'"
    ...
    def test_theme_css_generation(self, renderer, theme):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            success = renderer.export_html(
                content, str(output_path), standalone=True, include_css=True
            )
