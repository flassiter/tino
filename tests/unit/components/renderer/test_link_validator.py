...
"""
        ...
        ref1_link = next(
            link for link in reference_links if link["reference"] == "ref1"
        )
        assert ref1_link["url"] == "http://example.com"

        ref2_link = next(
            link for link in reference_links if link["reference"] == "ref2"
        )
...
"""
        ...
        missing_link_issue = next(
            issue for issue in broken_links if "missing.md" in issue.message
        )
...
"""
        ...
        broken_fragment = next(
            issue for issue in fragment_issues if "nonexistent-section" in issue.message
        )
...
"""
    ...
    def test_suggest_fragment_corrections(self, validator):
        ...
        headings = [
            ...
            Heading(level=2, text="Section One", id="section-one", line_number=5),
            Heading(
                level=2, text="Another Section", id="another-section", line_number=10
            ),
        ]
        ...
        suggestions = validator.suggest_fragment_corrections(
            "completely-different", headings
        )
    ...
    def test_performance_large_document(self, validator):
        ...
        for i in range(500):
            content_lines.append(
                f"Paragraph {i} with [link {i}](file{i}.md) and [ref {i}][ref{i}]."
            )
            content_lines.append("")
        ...
        assert (
            processing_time < 500
        ), f"Link finding took {processing_time:.2f}ms, should be <500ms"
